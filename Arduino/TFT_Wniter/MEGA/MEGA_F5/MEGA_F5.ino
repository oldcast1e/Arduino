...
// Store weather data for the next 3 hours in case of Wi-Fi disconnection
char storedWeatherData[3][16] = {"UNK", "UNK", "UNK"}; // Default to unknown
bool isStoredDataValid = false; // Flag to check if stored data is valid
// 전역 변수 선언 (기존 코드 상단에 추가)
String storedWeatherDataE[3] = {"UNK", "UNK", "UNK"};
...
void updateDisplay() {
    ...
    char tempBuffer[8];
    sprintf(tempBuffer, "%d C", (int)round(temperatureB)); // Use rounded integer
    ...
    for (int i = 0; i < 3; i++) {
        const unsigned char* bitmapE;

        if (hasError || storedWeatherDataE[i].length() == 0) {
            bitmapE = weather_error; // Error bitmap for missing data
        } else {
            bitmapE = mapWeatherToBitmap(storedWeatherDataE[i].c_str());
        }
...
void parseSerialInput(String input) {
    ...
    if (timeStr.length() == 4) {
        timeStr = "0" + timeStr; // Convert '9:01' to '09:01'
    }
    ...
    if (weatherData.startsWith("DataNotFound$")) {
        Serial.println("Warning: DataNotFound received. Checking for additional weather data...");

        // Trim "DataNotFound$" and continue parsing
        weatherData = weatherData.substring(weatherData.indexOf('$') + 1);
        if (weatherData.length() == 0) {
            Serial.println("Error: No additional weather data found after DataNotFound.");
            for (int i = 0; i < 3; i++) {
    ...
    // 4. Parse temperature (Section B)
    int tempStart = weatherData.indexOf("T:") + 2;
    if (tempStart > 1) { // Ensure "T:" exists
        int tempEnd = weatherData.indexOf('|', tempStart);
        if (tempEnd == -1) tempEnd = weatherData.length();
        String tempStr = weatherData.substring(tempStart, tempEnd);
        temperatureB = tempStr.toFloat(); // Update temperature
    } else {
        Serial.println("Warning: Missing temperature data.");
        temperatureB = -1; // Default temperature
    }

    // 5. Replace humidity with DHT11 value (Section C)
    ...
    // 6. Parse future weather data for 'E', 'F', 'G'
    for (int i = 0; i < 3; i++) {
        int futureHour = (currentHour + i + 1) % 24;  // Calculate future hour
        char futureKey[8];
        sprintf(futureKey, "$%02d00|W:", futureHour);  // Generate key (e.g., "$1000|W:")
