...
void updateDisplay() {
    ...
    mylcd.Print_String(currentDate, 10, 10); // Display current date and time
    ...
    char tempBuffer[8];
    if (temperatureB != -1) { // Check if temperature is valid
        sprintf(tempBuffer, "%d C", (int)temperatureB); // Display rounded temperature
    } else {
        sprintf(tempBuffer, "N/A"); // Display "N/A" if temperature is invalid
    }
...
void parseSerialInput(String input) {
    ...
    int tempStart = weatherData.indexOf("T:") + 2; // Find first "T:"
    if (tempStart > 1) { // Ensure "T:" exists
        int tempEnd = weatherData.indexOf('|', tempStart);
        if (tempEnd == -1) tempEnd = weatherData.length(); // If no '|', read until end
        String tempStr = weatherData.substring(tempStart, tempEnd);
        temperatureB = tempStr.toFloat(); // Update temperature as float
        if (isnan(temperatureB)) { // Handle parsing errors
            Serial.println("Error: Invalid temperature value.");
            temperatureB = -1;
        } else {
            temperatureB = round(temperatureB); // Round temperature to nearest integer
        }
    } else {
