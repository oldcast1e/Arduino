...
float temperatureB = 25;       // Default for Section B
...
void parseSerialInput(String input) {
    ...
    if (tIndex > 0) {
        ...
        if (tEnd == -1) tEnd = weatherData.length(); // Handle case where no '|' after temperature
        float tempValue = weatherData.substring(tIndex + 2, tEnd).toFloat(); // Convert substring to float

        // Round to nearest integer
        temperatureB = round(tempValue);

        Serial.print("Updated Temperature: ");
        Serial.println(temperatureB); // Debugging output to confirm correct update
    } else {
        Serial.println("Warning: Missing temperature data. Using default value.");
        temperatureB = 0; // Default value
    }
