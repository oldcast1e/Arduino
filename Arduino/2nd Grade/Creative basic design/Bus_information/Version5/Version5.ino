...
String getBusInfo(String arsId) {
  String url = String(apiEndpoint) + "getStationByUid?serviceKey=" + String(apiKey) + "&arsId=" + arsId;
  return sendGetRequest(url);
}
...
String getRouteByStationList() {
  String url = String(apiEndpoint) + "getRouteByStation?serviceKey=" + String(apiKey) + "&arsId=" + String(stationName);
  Serial.println("Requesting URL: " + url);  // 확인을 위해 요청하는 URL을 시리얼 모니터에 출력
  return sendGetRequest(url);
}
